...
class DummyHFClient:
    ...
    async def summarization(self, text, model, clean_up_tokenization_spaces=True, truncation=None, generate_parameters=None):
    ...
    async def translation(self, text, model, src_lang=None, tgt_lang=None, clean_up_tokenization_spaces=True, truncation=None, generate_parameters=None):
        return types.SimpleNamespace(translation_text="hola")

    async def image_classification(self, image_bytes, model, function_to_apply=None, top_k=1):
        return [types.SimpleNamespace(label="cat", score=0.8), types.SimpleNamespace(label="dog", score=0.2)]

    async def image_segmentation(self, image_bytes, model, mask_threshold, overlap_mask_area_threshold, subtask, threshold):
        return [types.SimpleNamespace(label="person", mask="ZmFrZS1iYXNlNjQ=")]

    async def text_to_image(self, prompt, model, guidance_scale, num_inference_steps, width, height, negative_prompt=None, seed=None, scheduler=None):
        return object()

    async def image_to_image(self, image_bytes, model, prompt, guidance_scale, negative_prompt, num_inference_steps, target_size):
        return object()

    async def text_to_speech(self, text, model, do_sample=None, temperature=None, top_k=None, top_p=None, max_new_tokens=None, num_beams=None, use_cache=None):
        return b"\x00\x01\x02"


@pytest.mark.asyncio
async def test_hf_text_classification(context: ProcessingContext):
    with patch("nodetool.nodes.huggingface_hub.Environment.get_environment", return_value={"HF_TOKEN": "x"}):
        with patch("nodetool.nodes.huggingface_hub.AsyncInferenceClient", DummyHFClient):
...
async def test_hf_summarization_translation_chat(context: ProcessingContext):
    with patch("nodetool.nodes.huggingface_hub.Environment.get_environment", return_value={"HF_TOKEN": "x"}):
        with patch("nodetool.nodes.huggingface_hub.AsyncInferenceClient", DummyHFClient):
            ...
            t = await Translation(text="hello", src_lang="en", tgt_lang="es").process(context)
            assert t == "hola"
...
async def test_hf_vision_and_audio_nodes(context: ProcessingContext):
    with patch("nodetool.nodes.huggingface_hub.Environment.get_environment", return_value={"HF_TOKEN": "x"}):
        with patch("nodetool.nodes.huggingface_hub.AsyncInferenceClient", DummyHFClient):
            ...
            async def _asset_to_bytes(_):
                return b"img"
            async def _image_from_base64(_):
                from nodetool.metadata.types import ImageRef
                return ImageRef(uri="mask://img")
            async def _image_from_pil(_):
                from nodetool.metadata.types import ImageRef
                return ImageRef()
            async def _audio_from_bytes(_):
            ...
            from nodetool.metadata.types import InferenceProvider, InferenceProviderTextToImageModel
            ...
            tts = await TextToSpeech(text="hello").process(context)
            # Returns AudioRef
            from nodetool.metadata.types import AudioRef
            assert isinstance(tts, AudioRef)
