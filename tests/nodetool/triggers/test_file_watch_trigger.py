...
from nodetool.nodes.nodetool.triggers import FileWatchTrigger
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            await asyncio.wait_for(
                asyncio.gather(create_file(), collect_events()), timeout=5.0
            )
...
async def test_file_watch_trigger_pattern_filter():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(events) == 1
        assert "capture.txt" in events[0]["path"]
...
@pytest.mark.asyncio
async def test_file_watch_trigger_setup_idempotent():
    """Test that repeated setup does not double-schedule the watch."""
    with tempfile.TemporaryDirectory() as tmpdir:
        trigger = FileWatchTrigger(
            id="test-trigger",
            path=tmpdir,
        ...
        try:
            await trigger.setup_trigger(context)
            await trigger.setup_trigger(context)
            assert trigger._observer is not None
        finally:
            await trigger.cleanup_trigger(context)
            await trigger.finalize(context)
