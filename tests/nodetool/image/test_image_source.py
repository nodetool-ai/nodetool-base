...
from nodetool.metadata.types import FontRef, ImageRef
...
async def test_image_source_nodes(context: ProcessingContext, node, expected_type):
    ...
    try:
        ...
        if isinstance(result, ImageRef):
            # ImageRef may have data or a URI - either is valid
            assert result.data is not None or result.uri is not None
