...
class HailuoImageToVideo(SingleOutputGraphNode[types.VideoRef], GraphNode[types.VideoRef]):
    """
    Generate videos from images using MiniMax's Hailuo model via Kie.ai.
    """

    ModelType: typing.ClassVar[type] = nodetool.nodes.kie.video.HailuoImageToVideo.ModelType
    Duration: typing.ClassVar[type] = nodetool.nodes.kie.video.HailuoImageToVideo.Duration
    Resolution: typing.ClassVar[type] = nodetool.nodes.kie.video.HailuoImageToVideo.Resolution
    ...
    image: types.ImageRef | OutputHandle[types.ImageRef] = connect_field(default=types.ImageRef(type='image', uri='', asset_id=None, data=None, metadata=None), description='The source image to animate.')
    model_type: nodetool.nodes.kie.video.HailuoImageToVideo.ModelType = Field(default=nodetool.nodes.kie.video.HailuoImageToVideo.ModelType.PRO, description='The model tier to use.')
    duration: nodetool.nodes.kie.video.HailuoImageToVideo.Duration = Field(default=nodetool.nodes.kie.video.HailuoImageToVideo.Duration.D6, description='The duration of the video in seconds. 10s is not supported for 1080p.')
    resolution: nodetool.nodes.kie.video.HailuoImageToVideo.Resolution = Field(default=nodetool.nodes.kie.video.HailuoImageToVideo.Resolution.R768P, description='The resolution of the video.')
...
class Sora2BaseNode(SingleOutputGraphNode[types.VideoRef], GraphNode[types.VideoRef]):
    ...
    remove_watermark: bool | OutputHandle[bool] = connect_field(default=True, description='Whether to remove the watermark from the video.')
    n_frames: int | OutputHandle[int] = connect_field(default=10, description='Number of frames for the video output.')
