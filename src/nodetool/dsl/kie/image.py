...
class QwenImageEdit(SingleOutputGraphNode[types.ImageRef], GraphNode[types.ImageRef]):
    ...
    Acceleration: typing.ClassVar[type] = nodetool.nodes.kie.image.QwenImageEdit.Acceleration
    ImageSize: typing.ClassVar[type] = nodetool.nodes.kie.image.QwenImageEdit.ImageSize
    NumImages: typing.ClassVar[type] = nodetool.nodes.kie.image.QwenImageEdit.NumImages
    OutputFormat: typing.ClassVar[type] = nodetool.nodes.kie.image.QwenImageEdit.OutputFormat
    ...
    acceleration: nodetool.nodes.kie.image.QwenImageEdit.Acceleration = Field(default=nodetool.nodes.kie.image.QwenImageEdit.Acceleration.NONE, description='Acceleration level for image generation. Higher acceleration increases speed.')
    image_size: nodetool.nodes.kie.image.QwenImageEdit.ImageSize = Field(default=nodetool.nodes.kie.image.QwenImageEdit.ImageSize.LANDSCAPE_4_3, description='The size of the generated image.')
    num_inference_steps: int | OutputHandle[int] = connect_field(default=25, description='The number of inference steps to perform (2-49).')
    seed: int | OutputHandle[int] = connect_field(default=-1, description='Random seed for reproducible results. Use -1 for random.')
    guidance_scale: float | OutputHandle[float] = connect_field(default=4.0, description='Classifier Free Guidance scale (0-20). Higher values stick closer to prompt.')
    num_images: nodetool.nodes.kie.image.QwenImageEdit.NumImages = Field(default=nodetool.nodes.kie.image.QwenImageEdit.NumImages.ONE, description='Number of images to generate.')
    enable_safety_checker: bool | OutputHandle[bool] = connect_field(default=True, description='Enable the safety checker for content filtering.')
    output_format: nodetool.nodes.kie.image.QwenImageEdit.OutputFormat = Field(default=nodetool.nodes.kie.image.QwenImageEdit.OutputFormat.PNG, description='The format of the generated image.')
    negative_prompt: str | OutputHandle[str] = connect_field(default='', description='Negative prompt for the generation (max 500 characters).')

    @classmethod
    def get_node_class(cls) -> type[BaseNode]:
        return nodetool.nodes.kie.image.QwenImageEdit
