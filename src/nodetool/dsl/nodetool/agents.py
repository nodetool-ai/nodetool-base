...
class Agent(GraphNode[nodetool.nodes.nodetool.agents.Agent.OutputType]):
    ...
    tools: list[types.ToolName] | OutputHandle[list[types.ToolName]] = connect_field(default=[], description='Tools to enable for the agent. Select workspace tools (read_file, write_file, list_directory) to enable file operations.')
...
class AgentOutputs(DynamicOutputsProxy):
    ...
    @property
    def thinking(self) -> OutputHandle[types.Chunk]:
        return typing.cast(OutputHandle[types.Chunk], self['thinking'])
