# This file is auto-generated by nodetool.dsl.codegen.
# Please do not edit this file manually.

# Instead, edit the node class in the source module and run the following commands to regenerate the DSL:
# nodetool package scan
# nodetool codegen

from pydantic import BaseModel, Field
import typing
from typing import Any
import nodetool.metadata.types
import nodetool.metadata.types as types
from nodetool.dsl.graph import GraphNode, SingleOutputGraphNode

import typing
from pydantic import Field
from nodetool.dsl.handles import OutputHandle, OutputsProxy, connect_field
import nodetool.nodes.search.bing
from nodetool.workflows.base_node import BaseNode

class BingImages(GraphNode[nodetool.nodes.search.bing.BingImages.OutputType]):
    """

        Search Bing Images to find visual content from across the web.
        bing, images, serp, visual, search
    """

    query: str | OutputHandle[str] = connect_field(default='', description='Search query or keyword for images')
    num_results: int | OutputHandle[int] = connect_field(default=20, description='Maximum number of image results to return')

    @property
    def out(self) -> "BingImagesOutputs":
        return BingImagesOutputs(self)

    @classmethod
    def get_node_class(cls) -> type[BaseNode]:
        return nodetool.nodes.search.bing.BingImages

    @classmethod
    def get_node_type(cls):
        return cls.get_node_class().get_node_type()

class BingImagesOutputs(OutputsProxy):
    @property
    def results(self) -> OutputHandle[list[dict]]:
        return typing.cast(OutputHandle[list[dict]], self['results'])

    @property
    def text(self) -> OutputHandle[str]:
        return typing.cast(OutputHandle[str], self['text'])

    @property
    def images(self) -> OutputHandle[list[types.ImageRef]]:
        return typing.cast(OutputHandle[list[types.ImageRef]], self['images'])


import typing
from pydantic import Field
from nodetool.dsl.handles import OutputHandle, OutputsProxy, connect_field
import nodetool.nodes.search.bing
from nodetool.workflows.base_node import BaseNode

class BingNews(GraphNode[nodetool.nodes.search.bing.BingNews.OutputType]):
    """

        Search Bing News to retrieve current news articles and headlines.
        bing, news, serp, articles, journalism
    """

    query: str | OutputHandle[str] = connect_field(default='', description='Search query or keyword for news articles')
    num_results: int | OutputHandle[int] = connect_field(default=10, description='Maximum number of news results to return')

    @property
    def out(self) -> "BingNewsOutputs":
        return BingNewsOutputs(self)

    @classmethod
    def get_node_class(cls) -> type[BaseNode]:
        return nodetool.nodes.search.bing.BingNews

    @classmethod
    def get_node_type(cls):
        return cls.get_node_class().get_node_type()

class BingNewsOutputs(OutputsProxy):
    @property
    def results(self) -> OutputHandle[list[dict]]:
        return typing.cast(OutputHandle[list[dict]], self['results'])

    @property
    def text(self) -> OutputHandle[str]:
        return typing.cast(OutputHandle[str], self['text'])


import typing
from pydantic import Field
from nodetool.dsl.handles import OutputHandle, OutputsProxy, connect_field
import nodetool.nodes.search.bing
from nodetool.workflows.base_node import BaseNode

class BingSearch(GraphNode[nodetool.nodes.search.bing.BingSearch.OutputType]):
    """

        Search Bing to retrieve organic web search results.
        bing, search, serp, web, query
    """

    query: str | OutputHandle[str] = connect_field(default='', description='Search query or keyword to search for')
    num_results: int | OutputHandle[int] = connect_field(default=10, description='Maximum number of results to return')

    @property
    def out(self) -> "BingSearchOutputs":
        return BingSearchOutputs(self)

    @classmethod
    def get_node_class(cls) -> type[BaseNode]:
        return nodetool.nodes.search.bing.BingSearch

    @classmethod
    def get_node_type(cls):
        return cls.get_node_class().get_node_type()

class BingSearchOutputs(OutputsProxy):
    @property
    def results(self) -> OutputHandle[list[dict]]:
        return typing.cast(OutputHandle[list[dict]], self['results'])

    @property
    def text(self) -> OutputHandle[str]:
        return typing.cast(OutputHandle[str], self['text'])


import typing
from pydantic import Field
from nodetool.dsl.handles import OutputHandle, OutputsProxy, connect_field
import nodetool.nodes.search._base
from nodetool.workflows.base_node import BaseNode

class SerpNode(SingleOutputGraphNode[typing.Any], GraphNode[typing.Any]):
    """
    Base class for SerpAPI-powered search nodes.

        Provides common provider initialization and the search_raw helper.
        Subclasses implement process() and call self._search_raw() instead of
        repeating the 8-line provider boilerplate.
    """

    @classmethod
    def get_node_class(cls) -> type[BaseNode]:
        return nodetool.nodes.search._base.SerpNode

    @classmethod
    def get_node_type(cls):
        return cls.get_node_class().get_node_type()


