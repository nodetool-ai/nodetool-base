...
class Sum(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> float:
        ...
        # ⚡ Bolt: Removed O(N) explicit type checking. EAFP (try/except) combined with
        # C-optimized `sum()` eliminates O(N) loop overhead and improves large list performance.
        try:
...
class Minimum(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> float:
        ...
        # ⚡ Bolt: Replaced O(N) explicit type checking with C-optimized `min()`.
        # EAFP ensures we only do a post-calculation type check on the O(1) result instead of O(N) upfront.
        try:
            res = min(self.values)
...
class Maximum(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> float:
        ...
        # ⚡ Bolt: Replaced O(N) explicit type checking with C-optimized `max()`.
        # EAFP ensures we only do a post-calculation type check on the O(1) result instead of O(N) upfront.
        try:
            res = max(self.values)
            if not isinstance(res, (int, float)):
