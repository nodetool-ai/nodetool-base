...
class Sum(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> float:
        ...
        # ⚡ Bolt: Using EAFP (try...except) with C-optimized built-in `sum()`
        # avoids the O(N) Python iteration overhead of explicit type checking
        # (e.g., `all(isinstance(x, ...))`). We check the final result's type
        # to ensure homogenous numerical output.
        try:
            res = sum(self.values)
...
class Minimum(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> float:
        ...
        # ⚡ Bolt: Using EAFP (try...except) with C-optimized built-in `min()`
        # avoids the O(N) Python iteration overhead of explicit type checking
        # (e.g., `all(isinstance(x, ...))`). We check the final result's type
        # to ensure homogenous numerical output.
        try:
...
class Maximum(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> float:
        ...
        # ⚡ Bolt: Using EAFP (try...except) with C-optimized built-in `max()`
        # avoids the O(N) Python iteration overhead of explicit type checking
        # (e.g., `all(isinstance(x, ...))`). We check the final result's type
        # to ensure homogenous numerical output.
        try:
...
class Product(BaseNode):
    ...
    async def process(self, context: ProcessingContext) -> float:
        ...
        # ⚡ Bolt: Replacing Python-level `reduce(lambda x,y: x*y, ...)` with
        # C-optimized `math.prod()` yields significant performance gains.
        # Using EAFP (try...except) avoids the O(N) Python iteration overhead
        # of explicit type checking (e.g., `all(isinstance(x, ...))`).
        try:
