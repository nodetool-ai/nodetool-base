...
class Flatten(BaseNode):
    ...
    def _flatten(self, lst: list[Any], current_depth: int = 0) -> Iterator[Any]:
        # Performance optimization: yield from generator avoids creating, copying,
        # and extending multiple intermediate lists during deep recursion,
        # yielding roughly O(N) performance instead of O(N^2) for deep nestings.
        for item in lst:
