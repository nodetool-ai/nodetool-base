...
## 2024-11-20 - Fast Pandas DataFrame Iteration
**Learning:** `df.iterrows()` in Pandas is a known performance bottleneck for iterating over rows because it yields a new Pandas Series object for every single row.
**Action:** When you need to iterate over DataFrame rows line-by-line without vectorization, use bulk dictionary conversion `zip(df.index, df.to_dict('records'))` to iterate through Python dictionaries instead of Series objects. This can provide up to a 20x speedup.
